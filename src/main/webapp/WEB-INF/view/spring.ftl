...
<#macro bind path>
    ...
    <#elseif status.value?exists && status.value?is_date>
        <#assign stringStatusValue=status.value?date>
    <#else>
...
<#macro formSingleSelectBill path options attributes="">
    <@bind path/>
    <select id="${status.expression}" name="${status.expression}" ${attributes}>
        <#list options.key as value>
        <option value="${value?html}"<@checkSelected value/>>${options.values[value_index]?html}</option>
        </#list>
    </select>
</#macro>

<#macro formMultiSelectBill path options attributes="">
    <@bind path/>
...
<#macro formSingleSelectWithBlank path options attributes="" first="">
    <@bind path/>
    <select id="${status.expression}" name="${status.expression}" ${attributes}>
...
<#macro formRadioButtons path options separator attributes="">
    <@bind path/>
    <#assign opValue = stringStatusValue>
    <#if stringStatusValue?is_number>
    	<#assign opValue = "" + stringStatusValue>
    </#if>
    <#list options?keys as value>
    <#assign id="${status.expression}${value_index}">
    <input type="radio" id="${id}" name="${status.expression}" value="${value?html}"
        <#if opValue == value>checked="checked"</#if> ${attributes}
    <@closeTag/>
 ...
 * @param attributes any additional attributes for the element (such as class
 *    or CSS styles or size
     checkbox1:${status.value?default([""])}
    <#assign isSelected = truecontains(status.value?default([""]), value)>
    checkbox:${isSelected}
-->
...
<#macro FormSingleCheckboxes path options separator="" attributes="">
    <@bind path/>
    <#list options?keys as value>
    <#assign id="${status.expression}${value_index}">
    <input type="checkbox" id="${id}" name="${status.expression}" value="${value?html}"
        <#if status.value==value>checked="checked"</#if> ${attributes}
    <@closeTag/>
    <label for="${id}">${options[value]?html}</label>${separator}
    </#list>
</#macro>
