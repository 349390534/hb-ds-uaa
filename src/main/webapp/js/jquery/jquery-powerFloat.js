...
(function($) {
	...
	var o = {
		...
		targetShow: function() {
			...
			//判断浏览器是否IE6,如是，则在‘selarea’末尾添加iframe，width\height自动获取selarea’首次显示的值
			if ($.browser.msie && $.browser.version < 7) {
				var popwidth = $(".selarea").width();
				var popheight = $(".selarea").height();
				$('<iframe style="position:absolute;left:0;top:0;z-index:-1;border:0 none;filter:alpha(opacity=0)"></iframe>').appendTo('.selarea').css({width:popwidth,height:popheight});
			};
			return this;
		},
		targetHide: function() {
