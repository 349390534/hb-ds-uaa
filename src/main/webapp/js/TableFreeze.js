/*******************************************************************************
 * * 名称:JQeury实现表格行列冻结 * * 作者:杨积江 * * 时间:2014年4月16日 * * 联系:yangjijiang@sina.cn * *
...
$.fn.mergeAttributes = function(src) {
	if ($.browser.msie) {
		...
		for (; i >= 0; i--) {
			...
			if (name.toLowerCase() === 'id' || attrs[i].value == ""
					|| attrs[i].value == null || attrs[i].value == "null") {
				continue;
			}
			try {
...
$.fn.FrozenTable = function(iRowHead, iRowFoot, iColLeft) {
	...
	if (oTable.width() > oDiv.width() && oTable.height() > oDiv.height()) {
		/*if (iRowHead > 0 && iColLeft > 0) {
			var oCloneTable = $("<table id='oTableLH_" + oTableId
					+ "'></table>");
		...
		/*if (iRowFoot > 0 && iColLeft > 0) {
			var oCloneTable = $("<table id='oTableLF_" + oTableId
					+ "'></table>");
			oDiv.parent().append(oCloneTable);
			oCloneTable.CloneTable(oTable, oTable.find("tr").length - iRowFoot,
					oTable.find("tr").length, iColLeft);
			oCloneTable.css("position", "absolute");
			oCloneTable.css("z-index", "1003");
			oCloneTable.css("left", oDiv.offset().left);
			oCloneTable.css("top", (oDiv.offset().top));
			oCloneTable.css("top", (oDiv.offset().top + oDiv.outerHeight(true)
					- oCloneTable.outerHeight(true) - 17));
		}*/
	}
	if (iRowHead > 0 && oTable.height() > oDiv.height()) {/*
		var oCloneDiv = $("<div id='oDivH_" + oTableId
				+ "'><table></table></div>");
	...
	*/}
	if (iRowFoot > 0 && oTable.height() > oDiv.height()) {/*
		var oCloneDiv = $("<div id='oDivF_" + oTableId
				+ "'><table></table></div>");
		oDiv.parent().append(oCloneDiv);
		oCloneDiv.find("table").CloneTable(oTable,
				oTable.find("tr").length - iRowFoot, oTable.find("tr").length,
				-1);
		oCloneDiv.css("overflow", "hidden");
		oCloneDiv.css("width", oDiv.outerWidth(true) - 17);
		oCloneDiv.css("position", "absolute");
		...
		oCloneDiv.css("left", oDiv.offset().left);
		oCloneDiv.css("top", oDiv.offset().top + oDiv.outerHeight(true)
				- oCloneTable.outerHeight(true) - 17);
		oCloneDiv.css("top", oDiv.offset().top);
	*/}
	if (iColLeft > 0 && oTable.width() > oDiv.width()) {
		oCloneDiv = $("<div id='oDivL_" + oTableId + "' name='oDivL_"+oTableId+"'><table></table></div>");
		oDiv.parent().append(oCloneDiv);
	...
	//oCloneDiv.find("table").find("tr td").css({"background-color": "#dbe3eb"});//
	oCloneDiv.find("table").find("tr td").css({"border": "1px solid #dbe3eb","background-color": "white"});//
	oCloneDiv.find("table").find("tr td.wdqd").css({"background-color": "#30a5ff","color": "white","font-weight": "bold"});
	...
	oDiv.scroll(function() {
		...
		if (typeof ($("#oDivL_" + oTableId).get(0)) != 'undefined') {
			...
			//加点效果
			//var oleft = oTable.find("tr:eq(0) td:eq(0)").outerWidth();
			oCloneDiv.show();
			var left = oDiv.scrollLeft();
			if(left==0){
				oCloneDiv.hide();
			}
		}
...
$.fn.CloneTable = function(oSrcTable, iRowStart, iRowEnd, iColumnEnd) {
	...
	for (var i = iRowStart; i < iRowEnd; i++) {
		...
		var colCount = 0;
		var colNumber = 0;
		for (var j = 0; j < (iColumnEnd == -1 ? oldTr.find("td").length
				: iColumnEnd); j++) {
		...
		Log += i + "==" + rowIndex + "=" + rowspanCount + "=" + rowNumber + "="
				+ rowspanValue + "<br>";
		if (i > rowIndex && i <= (rowIndex + rowspanValue - 1)
				&& iColumnEnd != -1) {
		...
		if (colNumber != 0) {
			...
			for (var j = 0; j < colNumber; j++) {
				...
				/*
				 * IE 一行多列合并时 if(iColumnEnd==-1 && iRowStart!=0 &&
				 * $.browser.msie){ if
				 * (typeof(newTd.attr("colspan"))!="undefined" &&
				 * newTd.attr("colspan")!=1) {
				 * alert(newTd.text()+"==2=="+newTd.attr("colspan")+"---"+colCount); } }
				 * if(iColumnEnd==-1 && iRowStart!=0 && j==1){
				 * newTd.width(oidTd.outerWidth(true)-1); jWidth +=
				 * (oidTd.outerWidth(true)-1); }else{
				 * newTd.width(oidTd.outerWidth(true)); jWidth +=
				 * oidTd.outerWidth(true); }
				 */
