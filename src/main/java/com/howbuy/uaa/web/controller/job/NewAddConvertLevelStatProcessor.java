 ...
 *
 */
public abstract class NewAddConvertLevelStatProcessor {
	
	public abstract Howbuy2EhowbuyIntervalStat createStatJob();

	private static Logger logger = LoggerFactory.getLogger(NewAddConvertLevelStatProcessor.class);
	...
	public String getJob() throws InterruptedException{
		...
		logger.info("get Job[{}] from NewAddConvertLevelStatProcessor,current queue size: {}",statDay,queue.size());
		return statDay;
	}

	public void addJob(String statDay) throws InterruptedException{
		queue.put(statDay);
		logger.info("put job[{}] to NewAddConvertLevelStatProcessor,current queue size: {}",statDay,queue.size());
	}
		...
		new Thread(){
			...
			public void run() {
				while(true){
					...
					try {
						statDay = getJob();
						Howbuy2EhowbuyIntervalStat statJob = createStatJob();
