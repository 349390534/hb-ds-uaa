...
public class HttpUtil {
	...
	static{
		...
		if(StringUtils.isBlank(appId)||StringUtils.isBlank(secretKey))
			throw new UaaRunTimeException("load appId,secretKey error");
	}
	...
	public String requestGet(String url){
		...
		try {
			...
			result = EntityUtils.toString(entity, UaaContants.CHARSET_UTF8);
		...
		try {
			if(null!=paramsMap && !paramsMap.isEmpty()){
				...
				for(Entry<String, String> en :sets){
					...
					value = UaaStringUtils.trimToEmpty(value);
